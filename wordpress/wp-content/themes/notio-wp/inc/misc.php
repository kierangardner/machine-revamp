...
function thb_social_article_detail( $btn = true ) {
	...
	if ( is_singular( 'post' ) ) {
		$sharing_type = ot_get_option( 'sharing_buttons', array( 'facebook', 'twitter', 'pinterest' ) );
	} elseif ( is_singular( 'portfolio' ) ) {
		$sharing_type = ot_get_option( 'sharing_buttons_portfolio', array( 'facebook', 'twitter', 'pinterest' ) );
	} elseif ( is_singular( 'product' ) ) {
		$sharing_type = ot_get_option( 'sharing_buttons_product', array( 'facebook', 'twitter', 'pinterest' ) );
	}
	?>
<aside class="share_wrapper">
	...
	if ( ! $btn ) {
		?>
		btn<?php } ?> share-post-link"><?php esc_html_e( 'Share', 'notio' ); ?></a>
			...
			<div class="small-12 large-10 columns">
				<div class="vcenter">
					<div class="product_share">
						...
						<a href="<?php echo 'http://www.facebook.com/sharer.php?u=' . urlencode( esc_url( $permalink ) ) . ''; ?>" rel="noreferrer nofollow" class="social facebook boxed-icon white-fill"><i class="fa fa-facebook"></i></a>
						<?php } ?>
						<?php if ( in_array( 'twitter', $sharing_type ) ) { ?>
						<a href="<?php echo 'https://twitter.com/intent/tweet?text=' . htmlspecialchars( urlencode( html_entity_decode( $title, ENT_COMPAT, 'UTF-8' ) ), ENT_COMPAT, 'UTF-8' ) . '&url=' . urlencode( esc_url( $permalink ) ) . '&via=' . urlencode( get_bloginfo( 'name' ) ) . ''; ?>" rel="noreferrer nofollow" class="social twitter boxed-icon white-fill"><i class="fa fa-twitter"></i></a>
						<?php } ?>
						<?php if ( in_array( 'pinterest', $sharing_type ) ) { ?>
						<a href="<?php echo 'http://pinterest.com/pin/create/link/?url=' . esc_url( $permalink ) . '&amp;media=' . ( ! empty( $image[0] ) ? $image[0] : '' ) . '&description=' . htmlspecialchars( urlencode( html_entity_decode( $title, ENT_COMPAT, 'UTF-8' ) ), ENT_COMPAT, 'UTF-8' ); ?>" rel="noreferrer nofollow" class="social pinterest boxed-icon white-fill" nopin="nopin" data-pin-no-hover="true"><i class="fa fa-pinterest"></i></a>
						<?php } ?>
						<?php if ( in_array( 'linkedin', $sharing_type ) ) { ?>
						<a href="<?php echo 'https://www.linkedin.com/cws/share?url=' . esc_url( $permalink ) . ''; ?>" rel="noreferrer nofollow" class="social linkedin boxed-icon white-fill"><i class="fa fa-linkedin"></i></a>
						<?php } ?>
						<?php if ( in_array( 'whatsapp', $sharing_type ) ) { ?>
						<a href="<?php echo 'whatsapp://send?text=' . htmlspecialchars( urlencode( html_entity_decode( $title, ENT_COMPAT, 'UTF-8' ) ), ENT_COMPAT, 'UTF-8' ) . ''; ?>" rel="noreferrer nofollow" class="whatsapp social boxed-icon white-fill" data-action="share/whatsapp/share"><i class="fa fa-whatsapp"></i></a>
						<?php } ?>
						<?php if ( in_array( 'vkontakte', $sharing_type ) ) { ?>
						<a href="<?php echo 'http://vk.com/share.php?url=' . esc_url( $permalink ) . ''; ?>" rel="noreferrer nofollow" class="social vk boxed-icon white-fill"><i class="fa fa-vk"></i></a>
						<?php } ?>
						<?php if ( in_array( 'email', $sharing_type ) ) { ?>
						<a href="<?php echo 'mailto:?Subject=' . htmlspecialchars( urlencode( html_entity_decode( $title, ENT_COMPAT, 'UTF-8' ) ), ENT_COMPAT, 'UTF-8' ) . ''; ?>" rel="noreferrer nofollow" class="email social boxed-icon white-fill"><i class="fa fa-envelope"></i></a>
						<?php } ?>
					</div>
...
function thb_site_bars() {
	...
	if ( $site_bars != 'off' ) {
					...
					<div class="thb-quick-inner">
						...
						foreach ( $site_bars_portfolio_list as $portfolio ) {
							...
							if ( $categories !== '' && ! empty( $categories ) ) {
								$thb_categories = wp_strip_all_tags( $categories );
							}
							?>
							<a class="quick-portfolio <?php echo esc_attr( $main_color_title ); ?>" id="qp-portfolio-<?php echo esc_attr( $id ); ?>" href="<?php echo esc_url( get_the_permalink( $portfolio['portfolio'] ) ); ?>">
		...
		?>
		<!-- Start Left Bar -->
		<aside id="bar-left" class="bar-side left-side site_bars_portfolio-<?php echo esc_attr( $site_bars_portfolio ); ?> <?php
		if ( $site_bars_portfolio_position == 'left' ) {
			echo esc_attr( 'active' ); }
		?>
		">
			<div class="abs"><?php echo wp_kses_post( $left_bar ); ?></div>
			<?php echo wp_kses_post( $site_bars_portfolio_position == 'left' ? $out : '' ); ?>
		</aside>
		<!-- End Left Bar -->

		<!-- Start Right Bar -->
		<aside id="bar-right" class="bar-side right-side site_bars_portfolio-<?php echo esc_attr( $site_bars_portfolio ); ?> <?php
		if ( $site_bars_portfolio_position == 'right' ) {
			echo esc_attr( 'active' ); }
		?>
		">
			<div class="abs right-side"><?php echo wp_kses_post( $right_bar ); ?></div>
			<?php echo wp_kses_post( $site_bars_portfolio_position == 'right' ? $out : '' ); ?>
		</aside>
		<!-- End Right Bar -->
		<?php
	}
	...
	if ( is_array( $socials ) ) {
		if ( count( $socials ) > 0 && '' !== $socials ) {
			foreach ( $socials as $social ) {
				?>
				<a href="<?php echo esc_url( $social['href'] ); ?>" class="<?php echo esc_attr( $social['social_network'] ); ?> icon-1x" rel="noreferrer nofollow" target="_blank">
					<i class="fa fa-<?php echo esc_attr( $social['social_network'] ); ?>"></i>
				<?php
				if ( $text ) {
					?>
					<span><?php echo esc_attr( $social['social_network'] ); ?></span><?php } ?>
				</a>
				<?php
			}
		}
...
function thb_blog_pagination() {
	...
	<div class="row pagination-space">
		...
		<?php
	} elseif ( $blog_pagination_style === 'style4' ) {
		?>
...
function thb_PostMeta() {
	...
	<aside class="post-bottom-meta hide">
		...
		<time class="date published time" datetime="<?php echo esc_attr( get_the_date( 'c' ) ); ?>" itemprop="datePublished" content="<?php echo esc_attr( get_the_date( 'c' ) ); ?>"><?php echo esc_html( thb_human_time_diff_enhanced() ); ?></time>
...
function thb_author( $id ) {
	...
	<div class="author-content">
		...
		<?php if ( get_the_author_meta( 'url', $id ) !== '' ) { ?>
			<a href="<?php echo esc_url( get_the_author_meta( 'url', $id ) ); ?>" rel="noreferrer nofollow" class="square-icon" target="_blank"><i class="fa fa-link"></i></a>
		<?php } ?>
		<?php if ( get_the_author_meta( 'twitter', $id ) !== '' ) { ?>
			<a href="<?php echo esc_url( get_the_author_meta( 'twitter', $id ) ); ?>" rel="noreferrer nofollow" class="square-icon twitter" target="_blank"><i class="fa fa-twitter"></i></a>
		<?php } ?>
		<?php if ( get_the_author_meta( 'facebook', $id ) !== '' ) { ?>
			<a href="<?php echo esc_url( get_the_author_meta( 'facebook', $id ) ); ?>" rel="noreferrer nofollow" class="square-icon facebook" target="_blank"><i class="fa fa-facebook"></i></a>
		<?php } ?>
	</div>
	</section>
		<?php
	...
	if ( $tags ) {
		...
		<aside class="related-posts cf hide-on-print">
			...
			while ( $related_posts->have_posts() ) :
				...
				get_template_part( 'inc/templates/postbits/related' );
endwhile;
			?>
			</div>
		</aside>
				...
				if ( $prev ) {
					...
					<a href="<?php echo get_permalink( $prev->ID ); ?>" class="post_nav_link prev">
						...
						<?php if ( is_singular( 'portfolio' ) ) { ?>
								<?php
								if ( $keyboard_nav === 'on' ) {
									esc_html_e( 'Previous Project (p)', 'notio' );
								} else {
									esc_html_e( 'Previous Project', 'notio' ); }
								?>
							<?php } elseif ( is_singular( 'product' ) ) { ?>
								<?php
								if ( $keyboard_nav === 'on' ) {
									esc_html_e( 'Previous Product (p)', 'notio' );
								} else {
									esc_html_e( 'Previous Product', 'notio' ); }
								?>
							<?php } else { ?>
								<?php
								if ( $keyboard_nav === 'on' ) {
									esc_html_e( 'Previous Post (p)', 'notio' );
								} else {
									esc_html_e( 'Previous Post', 'notio' ); }
								?>
							<?php } ?>
						</strong>
					...
					<a href="<?php echo get_permalink( $next->ID ); ?>" class="post_nav_link next">
						...
						<strong>
							<?php if ( is_singular( 'portfolio' ) ) { ?>
								<?php
								if ( $keyboard_nav === 'on' ) {
									esc_html_e( 'Next Project (n)', 'notio' );
								} else {
									esc_html_e( 'Next Project', 'notio' ); }
								?>
							<?php } elseif ( is_singular( 'product' ) ) { ?>
								<?php
								if ( $keyboard_nav === 'on' ) {
									esc_html_e( 'Next Product (n)', 'notio' );
								} else {
									esc_html_e( 'Next Product', 'notio' );}
								?>
							<?php } else { ?>
								<?php
								if ( $keyboard_nav === 'on' ) {
									esc_html_e( 'Next Post (n)', 'notio' );
								} else {
									esc_html_e( 'Next Post', 'notio' ); }
								?>
							<?php } ?>
						</strong>
...
function thb_footer_items() {
	if ( ot_get_option( 'scroll_totop' ) != 'off' ) {
		?>
...
function shortcode_empty_paragraph_fix( $content ) {
	$block = join( '|', array( 'thb_slidetype', 'thb_fadetype', 'thb_label' ) );
