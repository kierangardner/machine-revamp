...
function thb_portfolio_attributes( $style ) {
	$thb_id     = get_the_ID();
	$attributes = get_post_meta( $thb_id, 'attributes', true );
	?>
			...
			<?php if ( ! empty( $attributes ) ) { ?>
				<?php foreach ( $attributes as $more ) { ?>
					<div class="attribute">
						...
						<?php } ?>
						<?php echo wp_kses_post( $more['client_custom_value'] ); ?>
						<?php if ( $more['client_custom_link'] !== '' ) { ?>
							</a>
						<?php } ?>
					</div>
...
function thb_portfolio_categories( $categories, $id, $style, $portfolio_id_array = false ) {
	...
	<?php } ?>
	<nav class="thb-portfolio-filter <?php echo esc_attr( $style ); ?> <?php
	if ( $style === 'style1' ) {
		?>
		thb-fixed<?php } ?>" id="thb-filter-<?php echo esc_attr( $id ); ?>">
		<?php if ( $style === 'style1' ) { ?>
		...
		<ul class="filters">
			...
			foreach ( $categories as $cat ) {
				...
				echo '<li><a href="#" data-filter=".thb-cat-' . esc_attr( $term->slug ) . '" data-count="' . count( $posts ) . '">' . esc_html( $term->name ) . '</a></li>';
			}
			?>
			<?php if ( $style === 'style3' ) { ?>
