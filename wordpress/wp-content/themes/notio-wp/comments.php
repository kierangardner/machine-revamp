...
<?php if ( have_comments() ) : ?>
		<div class="row max_width">
			<div class="small-12 medium-10 large-8 medium-centered columns">
				<h4 class="comments-title">
					<?php
						printf(
							_nx( '%1$s Comment', '%1$s Comments', get_comments_number(), 'comments', 'notio' ),
							number_format_i18n( get_comments_number() )
						);
					?>
				</h4>
				<div class="text-center"><a href="#reply-title"><?php esc_html_e( 'LEAVE A REPLY', 'notio' ); ?></a></div>
	...
	if ( ! comments_open() ) :
		?>
	<p class="nocomments"><?php esc_html_e( 'Comments are closed', 'notio' ); ?></p>
	...
	$defaults = array(
		'fields'               => apply_filters(
			...
			array(
				...
				'url'    => '<div class="small-12 columns"><input name="url" size="30" id="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" type="text" class="placeholder" placeholder="' . __(
					'Website',
					'notio'
				) . '"/></div></div>',
			)
