	...
	SITE = {
		...
		menu: {
			...
			init: function() {
				...
				tlMainNav
					.to(menu, 0.4, {autoAlpha:1})
					.to(inner, 0.4, {x: 0})
					.staggerFrom(items, 0.3, { x: leftAligned * 50, opacity:0}, 0.1);
		...
		highlight: {
			...
			control: function(container, delay, skip) {
				if ( ( container.data('thb-in-viewport') === undefined ) || skip) {
					...
					TweenMax.to(container, 1, { backgroundSize: "+=100% +=0%", delay: delay});
				}
			}
		},
		slideType: {
		...
		iconbox: {
			...
			control: function(container, delay) {
				if( container.data('thb-in-viewport') === undefined ) {
					...
					if ( ! svg.length) {
						tl.from(_this.find('img'), animation_speed, { autoAlpha: 0, y: '20px'}, "s");
					} else {
						tl.set(svg, { display: 'block' })
							.staggerFrom(el, animation_speed, { drawSVG: "0%"}, 0.2, "s");
					}
					tl.staggerFrom(all, (animation_speed / 2), { autoAlpha: 0, y: '20px'}, 0.1, "s+="+ (animation_speed / 2) );
